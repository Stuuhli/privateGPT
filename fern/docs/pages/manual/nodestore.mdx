## NodeStores
PrivateGPT supports **Simple**, **SQLite**, and [Postgres](https://www.postgresql.org/) providers. Simple being the default.
...
SQLite offers the simplicity of the "simple" provider while persisting all metadata and embedded document information inside a single `.sqlite` database. This is especially useful when you want a portable file that can be inspected with standard SQL tooling instead of raw JSON files.

Enable it by setting the following in `settings.yaml`:
...
Once enabled, the ingested documents and their reference metadata will be written under `local_data/private_gpt/docstore.sqlite`, and index metadata will be stored in `local_data/private_gpt/indexstore.sqlite`.
